...
Vec3 FeetEndCal::calFootPos(int legID, Vec2 vxyGoalGlobal, float dYawGoal, float phase){
    _bodyVelGlobal = _est->getVelocity();//Vx
...
//未修改
Vec3 FeetEndCal::calFootPos(int legID, Vec2 vxyGoalGlobal, float dYawGoal, float phase, double _gaitHeight, double theta){
    _bodyVelGlobal = _est->getVelocity();//Vx
    _bodyWGlobal = _lowState->getGyroGlobal();//
    ...
    _footPos = _est->getPosition() + _nextStep;//roty(theta)*
    _footPos(2) = 0.0;
