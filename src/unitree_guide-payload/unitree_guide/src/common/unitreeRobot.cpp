...
}//获取leg0的足端在机身坐标系下的位置向量

Vec34 QuadrupedRobot::getVecXP(LowlevelState &state){
    Vec3 x = getX(state);//leg0的足端在机身坐标系下的位置向量，X点（p0）作为世界坐标系的原点
    Vec34 vecXP, qLegs;
    qLegs = state.getQ();//获取当前各个关节的角度

    for(int i(0); i < 4; ++i){
        vecXP.col(i) = _Legs[i]->calcPEe2B(qLegs.col(i)) - x;//Rsb(Pbi-Pb0)
    }
    return vecXP;
}//获取世界坐标系下各足端点的坐标位置向量
// Inverse Kinematics
...
}//获取足端在frame坐标系下的位置向量（单条腿）

// Forward derivative Kinematics 获取当前状态下足端速度向量（单条腿）
...
// Forward Kinematics 获取足端相对于机身中心的位置向量（四条腿）
