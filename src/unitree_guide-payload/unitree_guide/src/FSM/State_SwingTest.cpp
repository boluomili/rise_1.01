...
void State_SwingTest::enter(){
    ...
    _initFeetPos = _ctrlComp->robotModel->getFeet2BPositions(*_lowState, FrameType::HIP);//获取当前足端在基座坐标系下的位置
    _feetPos = _initFeetPos;
    _initPos = _initFeetPos.col(0);//取第一列，即右前腿（leg0）的足端位置
...
//根据目标足端位置，PD控制求出对足端的修正力，再转化为关节的前馈力矩
void State_SwingTest::_torqueCtrl(){
    Vec3 pos0 = _ctrlComp->robotModel->getFootPosition(*_lowState, 0, FrameType::HIP);//获取当前的足端位置
    Vec3 vel0 = _ctrlComp->robotModel->getFootVelocity(*_lowState, 0);//获取当前的足端速度

    Vec3 force0 = _Kp*(_posGoal - pos0) + _Kd*(-vel0);//对足端的修正力（假设足端的目标速度为0）

    Vec12 torque;
    Mat3 jaco0 = _ctrlComp->robotModel->getJaco(*_lowState, 0);//根据0号腿当前关节状态算雅可比矩阵

    torque.segment(0, 3) = jaco0.transpose() * force0;//计算出0号腿的关节力矩（前馈力矩）
