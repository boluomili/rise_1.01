...
void State_FixedStand::enter(){
    for(int i=0; i<4; i++){
        if(_ctrlComp->ctrlPlatform == CtrlPlatform::GAZEBO){
            _lowCmd->setSimStanceGain(i);//设置仿真时站立腿的刚度（位置刚度Kp、速度刚度Kd）
        }
        else if(_ctrlComp->ctrlPlatform == CtrlPlatform::REALROBOT){
            _lowCmd->setRealStanceGain(i);//设置实机时站立腿的刚度（位置刚度Kp、速度刚度Kd）
        }
        _lowCmd->setZeroDq(i);//将各关节期望速度设置为0
        _lowCmd->setZeroTau(i);//将各关节期望力矩设置为0
    }
    for(int i=0; i<12; i++){
        _lowCmd->motorCmd[i].q = _lowState->motorState[i].q;
        _startPos[i] = _lowState->motorState[i].q;//给电机设置的第一个期望角度都是目前所处的角度，存入_startPos
    }
...
void State_FixedStand::run(){
    ...
    _percent = _percent > 1 ? 1 : _percent;//_percent从0增到1
    for(int j=0; j<12; j++){
